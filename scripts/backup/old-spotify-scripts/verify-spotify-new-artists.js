      ...
      for (const artist of batch) {
        try {
          ...
          if (result.found) {
            ...
            if (verbose)
              console.log(
                `  ✅ found: ${result.spotifyData.name} (preserving scraped name: "${artist.name}")`,
              );
          } else {
  ...
  for (let i = 0; i < artists.length; i += BATCH_SIZE) {
    ...
    for (const artist of batch) {
      try {
        ...
        const result = {
          id: artist.id,
          name: artist.name, // ALWAYS preserve scraped name as per conservative policy
        ...
        if (verbose) {
          if (verificationResult.found) {
            console.log(
              `   ✅ ${artist.name} -> found: ${verificationResult.spotifyData.name} (preserving scraped name)`,
            );
          } else {
